...
func NewMemoryStore() *MemoryStore {
	...
	for i := 1; i <= 20; i++ {
		...
		id := fmt.Sprintf("CR-0716-%03d", 80+i)
...
func (s *SQLStore) Dashboard(ctx context.Context) (Dashboard, error) {
	var d Dashboard
	err := s.db.QueryRowContext(ctx, `SELECT COUNT(*), COALESCE(SUM(status='已发布'),0), COALESCE(SUM(status IN ('已排期','待制作','制作中')),0) FROM appointments`).Scan(&d.TodayAppointments, &d.Completed, &d.CheckedIn)
	if err != nil {
		return d, err
	}
...
func (s *SQLStore) CreateAppointment(ctx context.Context, a Appointment) (Appointment, error) {
	if a.ID == "" {
		a.ID = fmt.Sprintf("CR-%d", time.Now().UnixNano())
	}
	if a.Status == "" {
		a.Status = AppointmentPending
	}
