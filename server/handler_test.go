...
func TestRouterAppointmentLifecycleAndEnvelope(t *testing.T) {
	...
	body := bytes.NewBufferString(`{"patient":"选题《城市夜行》","department":"短视频","doctor":"林编辑"}`)
	...
	statusReq := httptest.NewRequest(http.MethodPost, "/api/v1/appointments/"+envelope.Data.ID+"/status", bytes.NewBufferString(`{"status":"已排期"}`))
	...
	illegalReq := httptest.NewRequest(http.MethodPost, "/api/v1/appointments/"+envelope.Data.ID+"/status", bytes.NewBufferString(`{"status":"已发布"}`))
	...
	if eventsRes.Code != http.StatusOK || !bytes.Contains(eventsRes.Body.Bytes(), []byte("已排期")) {
