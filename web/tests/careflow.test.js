...
test('CreatorFlow has appointment, schedule and followup data', async()=>{const source=await readFile(new URL('../src/main.js',import.meta.url),'utf8'); assert.match(source,/今日选题队列/); assert.match(source,/编辑排班/); assert.match(source,/复盘任务/); assert.match(source,/AP-0716-082/)})
